...
- Behavior authority is spec-driven: `specs/001-scan-ndi-sources/contracts/ndi-feature-contract.md` and `specs/001-scan-ndi-sources/tasks.md`.
...
- Use service-locator objects already in feature presentation (`SourceListDependencies`, `ViewerDependencies`) from `feature/ndi-browser/presentation/src/main/java/com/ndi/feature/ndibrowser/source_list/SourceListTelemetry.kt` and `feature/ndi-browser/presentation/src/main/java/com/ndi/feature/ndibrowser/viewer/ViewerTelemetry.kt`.
...
- Keep telemetry emission patterns in place (`SourceListTelemetry.kt`, `ViewerTelemetry.kt`, `ViewerRecoveryTelemetry.kt`).
- Keep retry semantics bounded to 15 seconds (`ViewerViewModel.kt`, `NdiViewerRepositoryImpl.kt`, `ViewerReconnectCoordinator.kt`).
...
- Respect toolchain baseline in `gradle/libs.versions.toml` (AGP 8.5.2, Gradle 8.7, Kotlin 1.9.24, SDK 34 with JDK 21 toolchain and Java 17 bytecode target) unless doing an intentional coordinated upgrade.
