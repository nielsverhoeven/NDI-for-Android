...
public partial class ViewerView : ContentView
{
    ...
    private void OnRenderTick(object? sender, EventArgs e)
    {
        ...
        // Second layer over the bridge's prologue blank (#408): Stop / CancelRetry / FailReconnect
        // all mean "do not paint", and this also covers the one case the bridge cannot — a pane
        // retired by ReleaseIfDisowned, where no stop happened at all because the frames now belong
        // to another ViewerViewModel and must keep flowing for *it*. Without this, that pane blanks
        // for a single frame on IsStopped and then repaints the new owner's live video under its own
        // "Stopped" badge (#348 / Nielsen #1, reached through the feature's own guard).
