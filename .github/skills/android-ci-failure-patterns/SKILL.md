...
description: >
  Reference guide for Android-specific CI failure classes encountered in this
  repository. Covers Mono Fast Deployment aborts, APK signature mismatches on
  cached emulators, stale Release build state, and the correct remediation for
  each. Use when diagnosing emulator-tests workflow failures, adb install errors,
  or libmonodroid crashes in CI or on a physical device.
---
