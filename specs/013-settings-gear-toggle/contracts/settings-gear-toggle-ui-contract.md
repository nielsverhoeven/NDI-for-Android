...
- The affordance MUST use gear/cog iconography only; wrench/manage iconography is prohibited for this settings action.
...
- While `settingsFragment` is visible, the settings screen MUST show a visible header/title indicating Settings.
- While `settingsFragment` is visible, the top-right gear affordance MUST remain visible and tappable (must not disappear).
...
- Rotation while settings is open MUST preserve the visible settings header/title.
