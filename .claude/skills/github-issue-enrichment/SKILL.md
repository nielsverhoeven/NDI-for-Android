...
description: Fetch open GitHub issues for the current repository, analyze one or more issues against the actual codebase, and rewrite the issue body with structured technical detail so it can serve as direct planning input for a future feature or migration. Use when asked to "enrich", "detail", "analyse" or "update" a GitHub issue, or when asked to "fetch open issues" and then improve their content.
allowed-tools: Bash
---
...
> yet and the caller is the main session, remind it to run the branch creation step
