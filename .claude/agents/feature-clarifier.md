...
description: Identifies underspecified areas in a feature request or spec and resolves them through up to 5 targeted questions. Writes the answers back to the GitHub issue and the local spec file. Use when asked to 'clarify a feature', 'resolve ambiguities', or when the main session or feature-planner delegates a clarification step.
...
2. Append to the GitHub issue via `github-issues-manager` (in Claude Code subagents cannot call other subagents, so the **main session** coordinates this handoff):
