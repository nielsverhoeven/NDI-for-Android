...
description: Translates an enriched GitHub issue into a structured feature spec and technical implementation plan for this .NET MAUI NDI app. GitHub is the single source of truth. Use when asked to 'plan a feature', 'create a spec', 'write a feature plan', or when the main session delegates Stage 2 of the feature pipeline.
...
If the issue body does not contain `<!-- enriched-by-copilot -->`, delegate enrichment to `github-issues-manager` before continuing. In Claude Code subagents cannot call other subagents, so the **main session** coordinates this handoff.
...
Scan for decisions that materially affect scope, architecture, or test design. If any exist, delegate to `feature-clarifier` before continuing (in Claude Code subagents cannot call other subagents, so the **main session** coordinates this handoff).
