...
- Deck-vs-sheet selection is `ViewerControlLayout.Choose(widthDp, heightDp)`
  (`src/Core/Features/Viewer/ViewerControlLayout.cs`), a pure Core policy: deck when the
  `ViewerView`'s own measured width is **≥640 dp AND** height is **≥470 dp** (measured via the
  view's own `SizeChanged`, not the window size class), else a bottom sheet.
- On hosts where the layout resolves to **Deck**: a **two-column Grid** — **Playback column**
  (status line; Smooth/Balanced/High quality segmented buttons; audio toggle; full-screen toggle;
  primary **Stop**; Retry/Cancel/Reconnect replace the quality row while reconnecting) and
  **Camera column** (d-pad ▲◄AF►▼ 3×3 grid of 48 dp buttons; vertical zoom rocker T/W of two 48 dp
  buttons beside it; preset grid of 8 buttons (2 rows × 4 columns) at 48 dp each — tap recalls,
  long-press (≥600 ms) stores with a short confirmation; endpoint status chip opening the existing
  `PtzEndpointPanel` dialog).
- **Camera column only shown when `IsPtzControlActive`**; otherwise Playback spans the full deck
  width — no empty column.
- On hosts where the layout resolves to **Sheet** (below the 640×470 dp threshold: phones in
  portrait, and phones in landscape below the threshold): the deck becomes a **bottom sheet**
  overlay with a drag handle and two MD3-style secondary tabs, **"Playback" / "PTZ"**,
  half-expanded by default, draggable/tappable to expand. The "PTZ" tab is hidden when
  `!IsPtzControlActive` (auto-falls back to "Playback").
- The tablet **in portrait** uses the pushed `ViewerPage` (single column, no source list pane) —
  whether it gets the deck or the sheet follows the same measured-size rule above, evaluated
  against the pushed page's own width and height.
...
- [x] Tablet landscape two-pane: with a VISCA endpoint saved, every control (status, quality,
...
- [x] Without PTZ (NDI source without PTZ, no endpoint): the deck shows only the playback column,
      no empty camera column.
- [x] Below the `ViewerControlLayout.Choose(widthDp, heightDp)` 640×470 dp threshold (phone
      portrait, or tablet portrait pushed `ViewerPage` measuring narrower/shorter than that — see
      the rule above): bottom sheet with tabs; PTZ tab shows d-pad, zoom, presets, endpoint chip
      without scrolling.
- [x] Full screen: overlay per wireframe A; controls hide after 3 s and return on tap; d-pad and
      zoom remain operable while visible; tally border visible.
- [x] Presets: tap recalls, long-press stores (with a short confirmation); the preset entry +
      Store/Recall buttons are removed.
- [x] All existing unit tests pass; new tests for ViewModel changes (preset recall/store
      parameterized commands, camera column visibility, store confirmation status text).
- [x] Device verification on the Galaxy Tab A9+ in landscape and portrait against
...
- Deck-vs-sheet selection via `ViewerControlLayout.Choose(widthDp, heightDp)`
  (`src/Core/Features/Viewer/ViewerControlLayout.cs`), driven by the `ViewerView`'s own measured
  size (no `IWindowSizeClassService` dependency).
...
- A new width-measurement *service*. `ViewerControlLayout.Choose(widthDp, heightDp)` is a pure,
  unit-tested `static` policy consuming the `ViewerView`'s own `SizeChanged`-reported width/height
  directly — deliberately not the window size class (`IWindowSizeClassService`), since that
  service tracks the whole window and would misclassify the narrower embedded pane on
  `SourceListPage`'s two-column split. Measuring the view's own size instead means the embedded
  pane (which can be as narrow as ~500 dp at the low end of the Expanded window class) correctly
  falls back to the sheet rather than being forced into a deck that doesn't fit.
